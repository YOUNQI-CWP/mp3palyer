...
void data_callback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
    ...
    ma_decoder_read_pcm_frames(g_audio_state.pDecoder, pOutput, frameCount, NULL);
}

// 'load_song' 现在只负责在设备停止时交换解码器
void load_song(int song_index) {
    ...
    // 2. 如果索引无效，则仅卸载
    if (song_index < 0 || song_index >= g_audio_state.playlist.size()) {
        return;
    }
    ...
    if (ma_decoder_init_file(filepath, NULL, pNewDecoder) != MA_SUCCESS) {
        delete pNewDecoder;
        return;
    }
...
int main(int argc, char* argv[]) {
    ...
    while (!quit) {
        ...
        float progress = 0.0f;
        ma_uint64 cursor = 0, length = 1;
        bool song_loaded = false;
        {
            std::lock_guard<std::mutex> lock(g_audio_mutex);
            if (g_audio_state.pDecoder) {
                song_loaded = true;
                ma_decoder_get_cursor_in_pcm_frames(g_audio_state.pDecoder, &cursor);
                ma_decoder_get_length_in_pcm_frames(g_audio_state.pDecoder, &length);
                if (length > 0) progress = (float)cursor / (float)length;
            }
        }
        ImGui::ProgressBar(progress);

        if (song_loaded && cursor >= length) {
            if (g_device_inited) ma_device_stop(&g_device);
            load_song(g_audio_state.current_song_index + 1);
            if (g_audio_state.current_song_index != -1 && g_device_inited) {
        ...
        auto switch_song = [&](int direction) {
            if (g_device_inited) ma_device_stop(&g_device);
            load_song(g_audio_state.current_song_index + direction);
            if (g_audio_state.current_song_index != -1 && g_device_inited) {
                ma_device_start(&g_device);
            }
        };

        if (ImGui::Button("<< Prev")) { switch_song(-1); }
        ImGui::SameLine();
        if (is_playing) {
